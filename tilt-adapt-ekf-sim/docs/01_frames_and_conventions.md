...
加速度计测量的是 **比力 (specific force)**，即支撑力除以质量。

```
a_meas = R_bn @ (a_lin_n + g_n) + b_a + n_a
```

- `a_meas`: 加速度计测量值 (m/s²)，即 **比力 (specific force)**
- `a_lin_n`: 非重力加速度（导航系）(m/s²)
- `g_n`: 重力向量（导航系）= [0, 0, +g] (m/s²)
- `R_bn`: 导航系到机体系的旋转矩阵
- `b_a`: 加速度计偏置 (m/s²)
- `n_a`: 加速度计噪声 (m/s²)
...
```
a_meas = R_bn @ g_n + b_a + n_a
```

**重要符号约定**：
- `g_n = [0, 0, +g]` (NED 坐标系，重力指向地心)
- 静止水平时，`a_meas = [0, 0, +g]` (比力指向天空)
- 这与物理直觉一致：加速度计测量的是"支撑力"，静止时支撑力向上抵消重力
