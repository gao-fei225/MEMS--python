...
def main():
    ...
    # 精细搜索 lambda_min 在 [0.98, 1.02] 范围
    print("\n精细搜索 lambda_min:")
    
    for lambda_min in [0.96, 0.97, 0.98, 0.99, 1.0, 1.01, 1.02, 1.03]:
        adaptive_cfg = {
            "Q_gyro": 1e-5,
            "Q_bias": 1e-8,
            "R0": 3.5e-6,
        ...
        for scenario, r in results.items():
            ...
            print(f"    {scenario}: 改善={r['improvement']:+.3f}% {status}")
        print(f"    全部优于: {'是' if all_better else '否'}, 最小改善: {min_improvement:+.3f}%")
    
    # 测试不同的 nis_low
    print("\n测试不同的 nis_low:")
    
    for nis_low in [2.0, 2.5, 3.0, 3.5, 4.0]:
        adaptive_cfg = {
            "Q_gyro": 1e-5,
            "Q_bias": 1e-8,
            "R0": 3.5e-6,
            "use_direction_meas": True,
            "innovation_stat": {"window_W": 30, "nis_high": 7.815, "nis_low": nis_low, "ewma_alpha": 0.05},
