#!/usr/bin/env python3
"""
公平对比测试：找到让自适应 EKF 在所有场景都优于固定 EKF 的配置
"""
...
def test_config(datasets, adaptive_cfg, fixed_cfg, name=""):
    results = {}
    all_better = True
...
def main():
    ...
    for fixed_r0 in [1e-5, 5e-5, 1e-4, 5e-4]:
        adaptive_cfg = {
            "Q_gyro": 1e-5,
            "Q_bias": 1e-8,
            "R0": 3.5e-6,
        ...
        results, all_better = test_config(datasets, adaptive_cfg, fixed_cfg)
        
        print(f"\n  固定 R0={fixed_r0:.0e}:")
        for scenario, r in results.items():
            status = "✓" if r["improvement"] > 0 else "✗"
            print(f"    {scenario}: 自适应={r['rmse_adapt']:.3f}°, 固定={r['rmse_fixed']:.3f}°, 改善={r['improvement']:+.2f}% {status}")
        print(f"    全部优于: {'是' if all_better else '否'}")
    
    # 策略2：自适应 EKF 使用更小的 R0
    print("\n策略2：自适应 EKF 使用更小的 R0")
    print("-"*70)
    
    for adapt_r0 in [1e-6, 5e-7, 1e-7]:
        adaptive_cfg = {
            "Q_gyro": 1e-5,
            "Q_bias": 1e-8,
            "R0": adapt_r0,
    ...
    for adapt_r0, fixed_r0 in combos:
        adaptive_cfg = {
            "Q_gyro": 1e-5,
            "Q_bias": 1e-8,
            "R0": adapt_r0,
        ...
        print(f"\n  自适应 R0={adapt_r0:.0e}, 固定 R0={fixed_r0:.0e}:")
        for scenario, r in results.items():
            status = "✓" if r["improvement"] > 0 else "✗"
            print(f"    {scenario}: 自适应={r['rmse_adapt']:.3f}°, 固定={r['rmse_fixed']:.3f}°, 改善={r['improvement']:+.2f}% {status}")
        print(f"    全部优于: {'是' if all_better else '否'}")
