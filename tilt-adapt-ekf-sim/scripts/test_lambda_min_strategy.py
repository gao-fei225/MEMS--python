...
def main():
    ...
    # 测试不同的 lambda_min
    print("\n测试不同的 lambda_min (R0=3.5e-6):")
    print("-"*70)
    
    for lambda_min in [1.0, 1.1, 1.2, 1.3, 1.5, 2.0]:
        adaptive_cfg = {
            "Q_gyro": 1e-5,
            "Q_bias": 1e-8,
            "R0": 3.5e-6,
    ...
    for r0, lambda_min in combos:
        ...
        print(f"\n  R0={r0:.0e}, lambda_min={lambda_min}:")
        for scenario, r in results.items():
            status = "✓" if r["improvement"] > 0 else "✗"
            print(f"    {scenario}: 自适应={r['rmse_adapt']:.3f}°, 固定={r['rmse_fixed']:.3f}°, 改善={r['improvement']:+.2f}% {status}")
        print(f"    全部优于: {'是' if all_better else '否'}")
