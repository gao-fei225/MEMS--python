...
def main():
    ...
    # 不同的 mag_threshold
    for mag_threshold in [0.05, 0.1, 0.2, 0.3]:
        adaptive_cfg = {
            "Q_gyro": 1e-5,
            "Q_bias": 1e-8,
            "R0": 3.5e-6,
    ...
    # 修改 EKF 代码以支持混合策略
    # 这里我们测试一个简单的方法：使用较小的 R0 + inflate
    for r0 in [1e-6, 2e-6, 3e-6]:
        adaptive_cfg = {
            "Q_gyro": 1e-5,
            "Q_bias": 1e-8,
            "R0": r0,
            "use_direction_meas": True,
            "innovation_stat": {"window_W": 30, "nis_high": 7.815, "nis_low": 3.0, "ewma_alpha": 0.05},
            "adaptation": {"lambda_max": 1000.0, "lambda_min": 1.0, "use_inflate_mapping": True, "inflate_decay_rate": 0.95},
            "dual_channel": {"enabled": False},
        }
